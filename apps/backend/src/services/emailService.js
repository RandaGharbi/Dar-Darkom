...
class EmailService {
    ...
    sendExportEmail(recipients, exportData, scheduleName) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (!fs_1.default.existsSync(exportData.filePath)) {
                ...
                const to = recipients.map(recipient => recipient.name ? `${recipient.name} <${recipient.email}>` : recipient.email).join(', ');
                ...
                const result = yield this.getTransporter().sendMail(mailOptions);
    ...
    sendErrorNotification(recipients, scheduleName, error) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const to = recipients.map(recipient => recipient.name ? `${recipient.name} <${recipient.email}>` : recipient.email).join(', ');
                const mailOptions = {
    ...
    generateEmailContent(exportData, scheduleName) {
        const typeNames = {
            sales: 'Ventes',
            products: 'Produits',
            customers: 'Clients',
    ...
    generateErrorEmailContent(scheduleName, error) {
        return `
      <!DOCTYPE html>
      <html>
      <head>
    ...
    verifyConnection() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                yield this.getTransporter().verify();
                return true;
            }
            catch (error) {
